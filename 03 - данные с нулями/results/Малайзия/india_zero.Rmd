---
title: "Моментум-стратегии и reality check Малайзия (данные с нулями)"
output:
  html_document:
      pandoc_args: [
      "+RTS", "-K64m",
...
resultDataFull <-  read.csv("stocks_malaysia.csv")
...
#mylist <- readRDS("brazil_zero_v1.RDS") # читаем из файла что там есть 
mylist <- readRDS("malaysia_zero_16.12.RDS") # читаем из файла что там есть 
V_star <- mylist[[4]]
V_bar <- mylist[[3]]
...
hist(best.strat*100,20,col="gray", labels = TRUE, xlab = "Среднемесячная доходность(%)", ylab = "Количество портфелей", main = "Распределение среднемесячных доходностей лучшей стратегии",ylim = c(0, 150))
...
```{r, echo=FALSE, results='asis'}
#mylist <- readRDS("brazil_zero_v1.RDS") # читаем из файла что там есть 
#mylist <- readRDS("china_zero_v1.RDS") # читаем из файла что там есть 
...
hist(best.strat*100,20,col="gray", labels = TRUE, xlab = "Среднемесячная доходность(%)", ylab = "Количество портфелей", main = "Распределение доходностей лучшей по t-stat стратегии",ylim = c(0, 150))
