...
## _____________________________Bonds_spain________________________
```{r, include=FALSE}
...
resultDataFull <-  read.csv(file=paste0(PATH,"data/spain_bonds.csv"), header=TRUE)
row.names(resultDataFull) <- resultDataFull[,1]
resultDataFull <-resultDataFull[,-1]
N <- (nrow(resultDataFull)-(2+UP3*4))%/%STEP 

mylist <- readRDS(paste(PATH, "results/spain_bonds_result.RDS",sep = "")) # читаем из файла что там есть 
