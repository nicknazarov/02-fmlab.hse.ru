...
resultDataFull <-  read.csv(file=paste0(PATH,"data/bonds.csv"), header=TRUE)
row.names(resultDataFull) <- resultDataFull[,1]
resultDataFull <-resultDataFull[,-1]
N <- (nrow(resultDataFull)-(2+UP3*4))%/%STEP 

mylist <- readRDS(paste(PATH, "results/result__russia_bonds_f0_2016-05-05_19:18:33.RDS",sep = "")) # читаем из файла что там есть 
...
>!!!!!!!! mean **`r mean(result_with_date_mean_max[,1])`** стратегий
