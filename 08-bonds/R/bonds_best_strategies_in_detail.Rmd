---
title: "Bonds Russia all (06.05.2016)"
output:
  html_document:
      pandoc_args: [
      "+RTS", "-K64m",
...
>Рассматривалось 4 процентили  **`r 0.5`**,  **`r 0.4`** ,  **`r 0.3`**,  **`r 0.2`**  ,  **`r 0.1`**     
 <p>
 <br />
 </p>  
...
resultDataFull <-  read.csv(file=paste0(PATH,"data/bonds_long.csv"), header=TRUE)
row.names(resultDataFull) <- resultDataFull[,1]
resultDataFull <-resultDataFull[,-1]
N <- (nrow(resultDataFull)-(2+UP3*4))%/%STEP 

mylist <- readRDS(paste(PATH, "results/bonds_result_long.RDS",sep = "")) # читаем из файла что там есть 
...
hist(result_with_date_mean_max[,1],20,col="gray", labels = TRUE, xlab = "Среднемесячная доходность", ylab = "Количество портфелей", main = "Распределение среднемесячных доходностей",ylim = c(0, 100))
...
hist(result_with_date_mean_min[,1],20,col="gray", labels = TRUE, xlab = "Среднемесячная доходность", ylab = "Количество портфелей", main = "Распределение среднемесячных доходностей",ylim = c(0, 100))
...
hist(result_with_date_t[,1],20,col="gray", labels = TRUE, xlab = "Среднемесячная доходность", ylab = "Количество портфелей", main = "Распределение среднемесячных доходностей",ylim = c(0, 100))
