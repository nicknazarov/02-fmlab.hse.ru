---
title: "Bonds finland (13.05.2016)"
output:
  html_document:
      pandoc_args: [
      "+RTS", "-K64m",
...
## _____________________________Bonds_finland________________________
```{r, include=FALSE}
...
resultDataFull <-  read.csv(file=paste0(PATH,"data/finland_bonds.csv"), header=TRUE)
row.names(resultDataFull) <- resultDataFull[,1]
resultDataFull <-resultDataFull[,-1]
N <- (nrow(resultDataFull)-(2+UP3*4))%/%STEP 

mylist <- readRDS(paste(PATH, "results/finland_bonds_result.RDS",sep = "")) # читаем из файла что там есть 
