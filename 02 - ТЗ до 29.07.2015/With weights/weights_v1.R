...
#temp <- ret.weight(4, 1, 4, STEP, N, resultDataFull[,-1], UP1, UP2, vect.weights) 
...
vect.weights <- c(7,6,5,3,2,1,-1,-2,-3,-5,-6,-7)/24
m <- 1
realityCheckData <- data.frame(1,1,1,1,1,1,1,1,1)
colnames(realityCheckData) <-c("mean","t","p-value","hist_per","moment_per","invest_per","winners","losers", "Amount_of_negative")
...
for (p1 in 1:UP1 ){
    for (p2 in 0:UP2 ){   
      for (p3 in 1:UP3 ){  
        #вектор дельт    
        temp <- ret.weight(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, vect.weights) 
        return.winner<- ret.winner(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, vect.weights[1:(length(vect.weights)/2)]) 
        return.loser<- ret.loser(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, vect.weights[(length(vect.weights)/2+1):length(vect.weights)]) 
        n <- length(temp)
        ...
        realityCheckData[m, ] <- list(mean(temp),abs(mean(temp))/sd(temp)*sqrt(n), (1-pt(q = abs(mean(temp))/sd(temp)*sqrt(n),df = n-1))*2 ,p1*4, p2, p3*4,
                                      mean(return.winner), mean(return.loser),length(temp[temp<0]))    
        cat(mean(temp),abs(mean(temp))/sd(temp)*sqrt(n), (1-pt(q = abs(mean(temp))/sd(temp)*sqrt(n),df = n-1))*2 ,p1*4, p2, p3*4, n, mean(return.winner), mean(return.loser),length(temp[temp<0]), "\n")
...
saveRDS(file = "weights_52_v1_(24).RDS",stuff) # сохраняем всё ценное в файл
mylist <- readRDS("weights_32_v1_(24).RDS") # читаем из файла что там есть 
