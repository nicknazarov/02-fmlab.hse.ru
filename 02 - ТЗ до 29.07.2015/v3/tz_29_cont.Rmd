...
mylist <- readRDS("tz_29_17.08_52_stocks.RDS") # читаем из файла что там есть 
V_star <- mylist[[4]]
V_bar <- mylist[[3]]
...
ggplot(data.plot2, aes(x))  +   geom_line(aes(y = delta, colour = "delta"),size=2, colour = "black",linetype="dashed") + 
  geom_line(aes(y = winner, colour = "winner"),size=4, colour = "red") + 
  geom_line(aes(y = mmvb, colour = "mmvb"),size=2, colour = "blue")+ theme(axis.text.x = element_text(angle = 60, hjust =1,size=17, face="bold"),
  axis.text.y = element_text(size=25,face="bold") ,axis.title=element_text(size=25,face="bold"),plot.title = element_text(color="blue", size=34, face="bold"))+ 
  scale_x_date(labels = date_format("%d/%m/%y"),breaks = date_breaks("1 week"))  + 
...
## 6.  Моментум стратегии при равных весах (бенчмарк индекс ММВБ) для компаний с максимальной рыночной капитализацией (всего 32 компании)
...
mylist <- readRDS("tz_29_cont_v1_with_sharp(Q=0.1)_32_stocks.RDS") # читаем из файла что там есть 
V_star <- mylist[[4]]
V_bar <- mylist[[3]]
result.data <- mylist[[1]]
...
sharp.ret_p <- c(V_star_sharp ,V_bar_sharp )
sharp.boot.pvalue <- 1-(rank(sharp.ret_p, ties.method = "first")[501]-1)/500
...
>При ранжировании по средней доходности bootstrap p-value равен  **`r boot.pvalue`**

<p>
<br />
</p> 

>При ранжировании по t-статистике bootstrap p-value равен  **`r sharp.boot.pvalue`**

<p>
<br />
</p> 

>Всего портфелей у лучшей по средней доходности стратегии -- **`r length(best.strat)`** 
>Количество отрицательных доходностей у лучшей по средней доходности стратегии -- **`r length(neg.fold)`** ( **`r round(length(neg.fold)/length(best.strat)*100,2)`%** )
