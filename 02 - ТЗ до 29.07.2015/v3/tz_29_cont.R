  ...
  for (p1 in 1:UP1 ){
    for (p2 in 0:UP2 ){   
      for (p3 in 1:UP3 ){  
        ...
        temp <- ret.winner(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, percent) 
        n <- length(temp)
        ...
        #############################################################REALITY_CHECK##########################################################################      
        temp <- temp-ret.mmvb(p1, p2, p3, STEP, N, mmvb.table[,8], UP1, UP2) 
        f_bar <- mean(temp)
...
saveRDS(file = "tz_29_cont_v1_with_sharp(Q=0.1)_32_stocks.RDS",stuff) # сохраняем всё ценное в файл
