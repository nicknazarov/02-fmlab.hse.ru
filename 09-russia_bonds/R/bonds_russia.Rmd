---
title: " Бонды по России ликвидные - победители (05.07.2016)"
output:
  html_document:
      pandoc_args: [
      "+RTS", "-K64m",
...
resultDataFull <-  read.csv(file=paste0(PATH,"R/bonds.csv"), header=TRUE)
row.names(resultDataFull) <- resultDataFull[,1]
resultDataFull <-resultDataFull[,-1]
N <- (nrow(resultDataFull)-(2+UP3*4))%/%STEP 

mylist <- readRDS(paste(PATH, "R/short_win_bonds_result.RDS",sep = "")) # читаем из файла что там есть 
