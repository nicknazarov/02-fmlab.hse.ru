...
if(flag_load =="c"){
  price_d5<- read.csv(file=paste0("",file_name),  header=TRUE, sep = ";", dec = ",")
}
if(flag_load =="p"){
  price_d5<- read.csv(file=paste0("",file_name),  header=TRUE)
}
...
clusterEvalQ(cl,source("reality_func2.R")) # загрузка функций в кластер
